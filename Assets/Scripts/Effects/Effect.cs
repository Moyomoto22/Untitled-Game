...
public class Effect : MonoBehaviour
{
    ...
    /// <returns></returns>
    public async UniTask<bool> CallEffect(Constants.EffectType type, CharacterStatus user, CharacterStatus objective)
    {
        bool result = true;
        ...
        switch (type)
        {
            case Constants.EffectType.Guard:                    // 物理ダメージ
                await Guard(objective);
                break;
            case Constants.EffectType.PhysicalDamage:           // 物理ダメージ
                await PhysicalDamage(user, objective);
                break;
            case Constants.EffectType.MagicalDamage:            // 魔法ダメージ
                await MagicalDamage(user, objective);
                break;
            case Constants.EffectType.HealHP:                   // HP回復
                await HealHP(user, objective);
                break;
            case Constants.EffectType.HealHpByItem:             // HP回復(アイテム)
                result = await HealHPByItem(user, objective);
                break;
            case Constants.EffectType.HealMpByItem:             // MP回復(アイテム)
